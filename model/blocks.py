...
class TriplaneConvs(nn.Module):
    ...
    def forward(self, tri_feats: list, tri_noises: list = None, add_noise=True, skip_add=True, time_emb: torch.Tensor=None):
        """
        Args:
        ...
        if add_noise and tri_noises is not None:
            tri_noises[0] = tri_noises[0].view(-1, *tri_noises[0].shape[3:])
            if yz_feat.shape[1] - tri_noises[0].shape[1] != 0:
                padding = torch.zeros((yz_feat.shape[0], yz_feat.shape[1] - tri_noises[0].shape[1], *tri_noises[0].shape[2:]), device=yz_feat.device)
                # yz_feat = yz_feat + torch.cat([tri_noises[0].repeat(yz_feat.shape[0], 1, 1, 1), padding], dim=1)
                yz_feat = yz_feat + torch.cat([tri_noises[0], padding], dim=1)
            else:
                yz_feat = yz_feat + tri_noises[0]
            if xz_feat.shape[1] - tri_noises[1].shape[1] != 0:
                tri_noises[1] = tri_noises[1].view(-1, *tri_noises[1].shape[3:])
                padding = torch.zeros((xz_feat.shape[0], xz_feat.shape[1] - tri_noises[1].shape[1], *tri_noises[1].shape[2:]), device=xz_feat.device)
                # xz_feat = xz_feat + torch.cat([tri_noises[1].repeat(xz_feat.shape[0], 1, 1, 1), padding], dim=1)
                xz_feat = xz_feat + torch.cat([tri_noises[1], padding], dim=1)
            else:
                xz_feat = xz_feat + tri_noises[1]
            if xy_feat.shape[1] - tri_noises[2].shape[1] != 0:
                tri_noises[2] = tri_noises[2].view(-1, *tri_noises[2].shape[3:])
                padding = torch.zeros((xy_feat.shape[0], xy_feat.shape[1] - tri_noises[2].shape[1], *tri_noises[2].shape[2:]), device=xy_feat.device)
                # xy_feat = xy_feat + torch.cat([tri_noises[2].repeat(xy_feat.shape[0], 1, 1, 1), padding], dim=1)
                xy_feat = xy_feat + torch.cat([tri_noises[2], padding], dim=1)
            else:
        ...
        # skip connect
        if skip_add:
            yz_feat = yz_feat + yz_feat_prev[:, 32:, ...]
            xz_feat = xz_feat + xz_feat_prev[:, 32:, ...]
            xy_feat = xy_feat + xy_feat_prev[:, 32:, ...]
