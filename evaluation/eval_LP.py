...
def main():
    ...
    result_lp_iou_percent_mean = np.mean(result_lp_iou_percent).round(6)
    result_lp_fscore_percent_mean = np.mean(result_lp_fscore_percent).round(6)
    
    eval_results = OrderedDict({'LP-IOU': result_lp_iou_percent_mean,
                                'LP-F-score': result_lp_fscore_percent_mean})
    ...
    # Pairing the values with names and then sorting based on values
    sorted_pairs = sorted(zip(result_lp_iou_percent, filenames), reverse=True)
    for (lp_iou, filename) in sorted_pairs:
        print(f"{filename}: {lp_iou}", file=fp)
