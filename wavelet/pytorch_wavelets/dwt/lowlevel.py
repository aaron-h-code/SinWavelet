...
def afb1d_cube(x, h0, h1, mode='zero', dim=-1):
    """ 1D analysis filter bank (along one dimension only) of an image
...
def sfb1d_cube(lo, hi, g0, g1, mode='zero', dim=-1):
    """ 1D synthesis filter bank of an image tensor
    """
    C = lo.shape[1]
    d = dim % 5
...
class AFB3D(Function):
    ...
    """
    # @staticmethod
    # def forward(ctx, x, h0_col, h1_col, h0_row, h1_row, h0_dep, h1_dep, mode):
    #     ctx.save_for_backward(h0_col, h1_col, h0_row, h1_row, h0_dep, h1_dep)
    #     ctx.shape = x.shape[-2:]
    #     mode = int_to_mode(mode)
    #     ctx.mode = mode
    #     lohi = afb1d_cube(x, h0_row, h1_row, mode=mode, dim=4)
    #     y = afb1d_cube(lohi, h0_col, h1_col, mode=mode, dim=3)
    #     y = afb1d_cube(y, h0_dep, h1_dep, mode=mode, dim=2)
    #     s = y.shape
...
class SFB3D(Function):
    ...
    def forward(ctx, low, highs, g0_row, g1_row, g0_col, g1_col, g0_dep, g1_dep, mode):
        ...
        ll = sfb1d_cube(low, llh, g0_dep, g1_dep, mode=mode, dim=2)
        lh = sfb1d_cube(lhl, lhh, g0_dep, g1_dep, mode=mode, dim=2)
        hl = sfb1d_cube(hll, hlh, g0_dep, g1_dep, mode=mode, dim=2)
        hh = sfb1d_cube(hhl, hhh, g0_dep, g1_dep, mode=mode, dim=2)
        lo = sfb1d_cube(ll, lh, g0_col, g1_col, mode=mode, dim=3)
        hi = sfb1d_cube(hl, hh, g0_col, g1_col, mode=mode, dim=3)
        y = sfb1d_cube(lo, hi, g0_row, g1_row, mode=mode, dim=4)
        return y
...
def prep_filt_afb2d(h0_col, h1_col, h0_row=None, h1_row=None, device=None):
    ...
    them as afb2d uses conv2d which acts like normal correlation.
