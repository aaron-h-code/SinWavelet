...
def calc_gradient_penalty(netD, real_data: torch.Tensor, fake_data: torch.Tensor, device: torch.device, dag=False, dag_idx=0):
    ...
    if dag == False:
        disc_interpolates = netD(interpolates, dag_idx=0)
    else:
        disc_interpolates = netD(interpolates, dag_idx=dag_idx)
...
def generate_tri_plane_noise(res_x: int, res_y: int, res_z: int, bz: int, J: int, nf: int, noise_amp: float, device: torch.device):
    ...
    noise = [(torch.randn(bz, J, 3, nf, res_y, res_z, device=device) * noise_amp).detach(),
             (torch.randn(bz, J, 3, nf, res_x, res_z, device=device) * noise_amp).detach(),
             (torch.randn(bz, J, 3, nf, res_x, res_y, device=device) * noise_amp).detach()]
