...
class SSGmodelTriplane(SSGmodelBase):
    ...
    def _netG_trainable_params(self, lr_g: float, lr_sigma: float, train_depth: int):
        ...
        parameter_list = [
            {"params": block.parameters(), "lr": lr_g * (lr_sigma ** (len(self.netG.body[-train_depth:]) - 1 - idx))}
    ...
    def _draw_fake_in_training(self, mode: str):
        ...
        if mode == 'rec':
            # print(mode, init_noise[0, 0, 0, 0, :])
            pass

        if self.scale < self.train_depth:
            fake = self.netG(init_noise, real_sizes, noises_list, mode, scale=self.scale)
        else:
            ...
            if mode == 'rec' and self.prev_opt_feats is not None:
                ...
                with torch.no_grad():
                    prev_feats = self.netG.draw_feats(
                        init_noise, real_sizes[:prev_depth + 1], noises_list[:prev_depth + 1],
                        mode, prev_depth + 1, scale=self.scale)
            ...
            fake = self.netG.decode_feats(
                prev_feats, real_sizes[prev_depth + 1:], noises_list[prev_depth + 1:], mode, prev_depth + 1, -1)
        ...
        for i in range(scale + 1):
            if i == 0:  # first scale no additive tri-plane noise
                ...
                if mode == 'rec':
                    ...
                    # tri_noise = generate_tri_plane_noise(*noise_shape, self.config.feat_dim * 2, self.noiseAmp_list[i], self.device)
                    tri_noise = generate_tri_plane_noise(*noise_shape, self.config.batch_size,
                                                         self.netG.dwt_forward[i - 1].J, self.config.feat_dim * 2,
                                                         self.noiseAmp_list[i], self.device)
    ...
    def generate(self, mode: str, scale=None, resize_factor=(1.0, 1.0, 1.0), upsample=1, return_each=False):
        ...
        out = self.netG(init_noise, real_sizes, noises_list, mode, coords, return_each=return_each, scale=self.scale)
